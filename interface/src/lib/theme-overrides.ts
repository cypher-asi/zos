 ...
 * Settings > Appearance editor. The accent token is included alongside the
 * chrome tokens so power users can dial in an off-palette accent without
 * waiting for named presets. All of these are applied as inline
 * `document.documentElement.style` rules, which beat anything declared on
 * `:root` / `[data-theme='...']` by specificity.
 */
...
const STORAGE_KEY = "zos-theme-overrides";
